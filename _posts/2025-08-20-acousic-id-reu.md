...
What do we do with these vectors? High dimension spaces are hard to visualize for humans, so we use the Uniform Manifold Approximation and Projection (UMAP) algorithm to reduce the vectors to 2D. We then tried various 2D clustering methods, with HDBSCAN and Gaussian Mixture Models as the most successful methods. We test by comparing to known classifications, such as time of day, degraded/non-degraded status. We can then put these clusters into our dataset as labels for our network visualization.

{% include 
    img_caption.html
    src="assets/acoustic_species_id/2025-08-20_clusters.png"
    caption="Clusters colored by known labels (left) and HDBSCAN labels (right)"
%}
